...
let data = {
	...
	start: (video_detail) => {
		...
		if (video_detail.items[0].liveStreamingDetails){

		}else{
			data.get(video_detail.items[0].id).then((res) => {
				for (let i of res) {
					fbid.data.push(i);
				}
				data.finish(fbid);
	...
	get: (id) => {
		return new Promise((resolve, reject) => {
			let datas = [];
			let promise_array = [];
			let api_url = `https://www.googleapis.com/youtube/v3/commentThreads?part=snippet%2Creplies&maxResults=100&order=time&videoId=${id}&key=${config.key}`;
...
let table = {
	generate: (rawdata) => {
		...
		let host = `https://www.youtube.com/watch?v=${rawdata.detail.id}`;

		for (let [j, val] of filterdata.entries()) {
			let picture = '';
			let tar = val.snippet.topLevelComment.snippet;
...
let filter = {
	...
	unique: (data) => {
		...
		data.forEach(function (item) {
			let key = item.snippet.topLevelComment.snippet.authorChannelUrl;
	...
	word: (data, word) => {
		let newAry = $.grep(data, function (n, i) {
			if (n.snippet.topLevelComment.snippet.textOriginal.indexOf(word) > -1) {
				return true;
			}
		});
		return newAry;
	...
	time: (data, st, t) => {
		...
		let newAry = $.grep(data, function (n, i) {
			let created_time = moment(n.snippet.topLevelComment.snippet.publishedAt)._d;
