...
var comment_detail = {
  show: function show(fbid) {
    var tar = data.filtered.find(function (item) {
      return item.id == fbid;
    });
    console.log(tar);
    var host = 'https://www.facebook.com/';
    ...
    tar.comments.data.forEach(function (item) {
      if (item.from) {} else {
        item.from = {
          name: item.id,
          id: item.id
    ...
    var _iterator4 = _createForOfIteratorHelper(tar.comments.data),
        _step4;
